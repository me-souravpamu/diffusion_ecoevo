...
for j=1:length(e)
    ...
    % First term
term1 = (fu_num + gv_num + (1 + sigma) * epsilon * Lam); %%% -trace

% Square root term
term2 = sqrt(4 * fv_num * gu_num + (fu_num - gv_num + (1 - sigma) * epsilon * Lam)^2); %% root(tr^2-4det)


lambda_plus = (term1 + term2)/2;  %%maximum eigen value = (-trace+sqrt(trace^2 - 4* deter))/2
%%%%%%%%%%%%%%%


data2=[data2; Lam real(lambda_plus)]; %%%%%%% storing the dispersion rate for each eigenvalues of the Laplacian
end
    data3=[data3;sigma max(data2(:,2))]; %%%%%%% storing the maximal dispersion rate (MSF) for each sigma values.
                                     %%%%%%%%%% the sigma value for which
                                     %%%%%%%%%% MSF will become positive
                                     %%%%%%%%%% from negative will give
...
plot([min(sigma_array), max(sigma_array)],[0,0])
